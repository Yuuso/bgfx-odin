...
api_thread_proc :: proc(
    ...
    cube_vertices_size := cast(c.uint32_t) (
        size_of(cube_vertices[0]) * len(cube_vertices))
    ...
    cube_indices_size := cast(c.uint32_t) (
        size_of(cube_indices[0]) * len(cube_indices))
...
main :: proc() {
    ...
    for bgfx.render_frame() != bgfx.Render_Frame.NoContext {}
}
...
load_program :: proc(
    ...
    load_shader :: proc(
        ...
        bgfx.set_shader_name(handle, strings.unsafe_string_to_cstring(name),
...
set_allocator_vtable :: proc(
    ...
    allocator_vtable.realloc = proc "c" (
        ...
        if size > 0 {
            ...
            //fmt.printfln("alloc  %16v % -8v %v:%v",
            //  raw_data(bytes), size, file, line)
        }
